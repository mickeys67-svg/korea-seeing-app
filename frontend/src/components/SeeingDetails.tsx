...
const SeeingDetails: React.FC<SeeingProps> = ({ data }) => {
    ...
    const metricsConfig = React.useMemo(() => [
        ...
        },
        {
            key: 'cloudCover',
            label: 'Cloud',
            icon: <Cloud className="w-5 h-5 mb-1 text-gray-400" />,
            value: scores.cloudCover,
            unit: '/8',
            subtext: null,
            modal: {
                title: "Cloud Cover",
        ...
        },
        {
            key: 'wind',
            label: 'Wind',
            icon: <Wind className="w-5 h-5 mb-1 text-gray-400" />,
            value: scores.wind,
            unit: '/8',
            subtext: null,
            modal: {
        ...
        },
        {
            key: 'jetStream',
            label: 'Jet Stream',
            icon: <Plane className="w-5 h-5 mb-1 text-gray-400 rotate-45" />,
            value: scores.jetStream,
            unit: '/8',
            subtext: null,
            modal: {
        ...
        },
        {
            key: 'convection',
            label: 'Convection',
            icon: <ThermometerSun className="w-5 h-5 mb-1 text-gray-400" />,
            value: scores.convection,
            unit: '/8',
            subtext: null,
            modal: {
            ...
            <div className="bg-gray-800 p-6 rounded-2xl shadow-lg w-full max-w-md">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {metricsConfig.map((metric, idx) => (
                        <div
                            key={idx}
                            className="bg-gray-700 p-3 rounded-xl flex flex-col items-center cursor-pointer hover:bg-gray-600 transition-colors active:scale-95 text-center"
                            onClick={() => setSelectedMetric(metric.modal)}
                        >
                            {metric.icon}
                            <span className="text-gray-400 text-xs mb-1">{metric.label}</span>
                            <span className={`text-xl font-bold ${getComponentColor(metric.value)}`}>
                                {metric.value}
